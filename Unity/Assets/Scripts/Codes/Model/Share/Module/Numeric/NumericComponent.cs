...
namespace ET
{
        ...
        public static void Insert(this NumericComponent self, int numericType, long value, bool isPublicEvent = true)
        {
            ...
            if (isPublicEvent)
            {
                EventSystem.Instance.Publish(self.DomainScene(),
                    new EventType.NumbericChange() { Unit = self.Parent, New = value, Old = oldValue, NumericType = numericType });
            }
        }
    ...
    public class NumericComponent : Entity, IAwake, ISerializeToEntity, ITransfer
    {
