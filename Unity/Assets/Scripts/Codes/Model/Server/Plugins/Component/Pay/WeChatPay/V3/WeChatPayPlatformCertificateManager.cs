...
namespace ET.Pay.WeChatPay.V3
{
    ...
    {
        private readonly ConcurrentDictionary<string, WeChatPayPlatformCertificate> _certs = new ConcurrentDictionary<string, WeChatPayPlatformCertificate>();
        ...
        public async ETTask<WeChatPayPlatformCertificate> GetCertificateAsync(IWeChatPayClient client, WeChatPayOptions options, string serial)
        {
        ...
        /// <param name="options">配置选项</param>
        /// <returns>微信支付平台证书信息</returns>
        public async ETTask<WeChatPayPlatformCertificate> GetCertificateAsync(IWeChatPayClient client, WeChatPayOptions options)
        {
            // 如果证书已缓存，则直接使用缓存的证书
