...
{
    [HttpHandler(SceneType.GmWeb, "/GMSearchClientErrorLog")]
    public class Http_PostGMSearchClientErrorLogHandler: HttpPostHandler<C2G_GMSearchClientErrorLog, G2C_GMSearchClientErrorLog>
    {
        protected override async ETTask Run(Entity domain, C2G_GMSearchClientErrorLog request, G2C_GMSearchClientErrorLog response, long playerid)
        {
            ...
            while (start < end)
            {
                dbCollectName.Add(DBLogManagerComponent.Instance.GetDBCollectionName("", (int)DBLogger.EDBLogLevel.ClientError, start));
