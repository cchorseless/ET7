...
export class WearableConfigRecord {
    ...
    constructor(_json_: any) {
        ...
        if (_json_.item_slot === undefined) { throw new Error() }
        this.itemSlot = _json_.item_slot
        if (_json_.item_type_name === undefined) { throw new Error() }
        this.itemTypeName = _json_.item_type_name
        if (_json_.model_player === undefined) { throw new Error() }
        ...
        if (_json_.skip_model_combine === undefined) { throw new Error() }
        this.skipModelCombine = _json_.skip_model_combine
        if (_json_.asset_modifier === undefined) { throw new Error() }
        this.assetModifier = _json_.asset_modifier
        if (_json_.control_point === undefined) { throw new Error() }
        this.controlPoint = _json_.control_point
        if (_json_.styles === undefined) { throw new Error() }
